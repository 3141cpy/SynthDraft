...
**当前阶段：P0-P2 全部完成 + 多轮质量优化（V2-V4 验证、LLM 幻觉优化、前端 polish、CodeRabbit 全方位审查 0 issues）**
...
| OCR | PaddleOCR | 3.7.0 |
| Embedding | bge-m3（FlagEmbedding） | 1.3.3 |
| 默认 LLM | qwen2.5-coder:7b（Ollama） | - |
...
- **Linux 端**：AI 服务（FastAPI 后端 + Celery Worker）+ 基础设施（PostgreSQL / Redis / Qdrant / MinIO / Ollama）。无状态、可水平扩展。
...
│   ├── src/
│   │   ├── app/               # App Router 页面（/ /review /generate /kb）
...
| Ollama API | http://localhost:11434 | 本地 LLM/VLM 推理 |
...
其他迭代规格位于 `.trae/specs/` 下（V2-V4 验证、LLM 幻觉优化、前端 polish、CodeRabbit 全方位审查等）。
